...
gcinclude.AliasList = T{'gcmessages','wsdistance','setcycle','dt','PerpRing','th','kite','meleeset','gcdrain','gcaspir','nukeset','burst','weapon','elecycle','helix','weather','nuke','death','fight','sir','tankset','proc','cj','pupmode','tpgun','cormsg','forcestring','siphon','warpring','xpring','warpstaff','telering','rrset','craftset','zeniset','fishset'};
...
function gcinclude.SetVariables()
	...
	gcdisplay.CreateToggle('PerpRing', false);
...
function gcinclude.HandleCommands(args)
	...
	elseif (args[1] == 'PerpRing') then
		gcdisplay.AdvanceToggle('PerpRing');
		toggle = 'TH Set';
		status = gcdisplay.GetToggle('PerpRing');
	...
	elseif (args[1] == 'xpring') then
		gcinclude.DoXpRing();
	elseif (args[1] == 'warpstaff') then
		gcinclude.DoWarpStaff();
...
function gcinclude.DoWarpRing()
	AshitaCore:GetChatManager():QueueCommand(1, '/lac disable main');
	AshitaCore:GetChatManager():QueueCommand(1, '/lac equip Main "Warp Cudgel"');
...
function gcinclude.DoXpRing()
	AshitaCore:GetChatManager():QueueCommand(1, '/lac disable Ring2');
	AshitaCore:GetChatManager():QueueCommand(1, '/lac equip Ring2 "Chariot Band"');
	...
	local function usering()
		...
		AshitaCore:GetChatManager():QueueCommand(1, '/item "Chariot Band" <me>');
		forceidleset:once(8);
	end
	
	usering:once(33);
end
function gcinclude.DoWarpStaff()
	AshitaCore:GetChatManager():QueueCommand(1, '/lac disable main');
...
function gcinclude.DoWarpStaff2()
	AshitaCore:GetChatManager():QueueCommand(1, '/lac equip Main "Trick Staff II"');

	local function usering()
		local function forceidleset()
			AshitaCore:GetChatManager():QueueCommand(1, '/lac set Idle');
		end
		AshitaCore:GetChatManager():QueueCommand(1, '/item "Trick Staff II" <me>');
		forceidleset:once(8);
	end
